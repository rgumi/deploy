...
func NewDialContextFunc(optFuncs ...dialerOpt) func(context.Context, string, string) (net.Conn, error) {
	opts := &dialerOpts{name: "test", monitoring: true, parentDialContextFunc: (&net.Dialer{}).DialContext}
...
func dialClientConnTracker(ctx context.Context, network string, addr string, dialerName string, opts *dialerOpts) (net.Conn, error) {
	...
	if opts.monitoring {
		log.Debugf("Attempting connection with %s", dialerName)
	}
	...
	if err != nil {
		...
		if opts.monitoring {
			log.Debugf("Failed connection with %s", dialerName)
		}
		return nil, err
	}
	if event != nil {
	...
	if opts.monitoring {
		log.Debugf("Successful connection with %s", dialerName)
	}
...
func (ct *clientConnTracker) Close() error {
	...
	if ct.opts.monitoring {
		log.Debugf("Closed connection with %s", ct.dialerName)
	}
	return err
}
